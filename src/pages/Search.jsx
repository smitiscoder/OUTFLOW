...
const getIconForCategory = (category) => {
  ...
  return categoryIcons[key] || categoryIcons['others']; // Return the icon or default to 'others' if no match
};

const Search = () => {
  const [searchTerm, setSearchTerm] = useState("");
  const [userExpenses, setUserExpenses] = useState([]); // 👈 Store user-specific expenses
  const auth = getAuth();
  const db = getFirestore();

  // 🔥 Fetch ONLY the current user's expenses (once on component mount)
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setUserExpenses(expenses); // 👈 Store all user expenses
    });

    return () => unsubscribe();
  }, []);

  // 🔍 Filter expenses based on search term (from userExpenses)
